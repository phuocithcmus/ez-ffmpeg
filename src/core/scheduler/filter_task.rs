...
pub(crate) fn filter_graph_init(
    ...
    let result = std::thread::Builder::new().name(format!("filtergraph{fg_index}")).spawn(move || {
        let mut graph: *mut AVFilterGraph = null_mut();
        let mut fgp = FilterGraphParameter::default();
        let node = filter_node.as_ref();
        let SchNode::Filter { inputs, best_input } = node else { unreachable!() };
        ...
        loop {
            ...
            unsafe {
                if ifps[input_index].media_type
        ...
        let mut ret = 0;
        for ofp in &mut ofps {
            ret = unsafe { fg_output_frame(&mut fgp, ofp, null_frame(), &frame_pool) };
            if ret < 0 {
                set_scheduler_error(
                    &scheduler_status,
                    &scheduler_result,
...
unsafe fn fg_send_frame(
    ...
    if need_reinit != 0 || (*graph).is_null() {
        ...
        if !ifilter_has_all_input_formats(ifps) {
            &mut ifps[input_filter_index].frame_queue.push_back(frame_box);
            return Ok(());
        }
        ...
        if !(*graph).is_null() {
            ...
            if need_reinit & AUDIO_CHANGED != 0 {
                ...
                av_bprintf(
                    &mut reason,
                    CString::new("audio parameters changed to %d Hz, \0")
                        .unwrap()
                        .as_ptr(),
                    (*frame).sample_rate,
                );
            ...
            if need_reinit & VIDEO_CHANGED != 0 {
                ...
                av_bprintf(
                    &mut reason,
                    CString::new("video parameters changed to %s(%s, %s), %dx%d, \0")
                        .unwrap()
                        .as_ptr(),
            ...
            if need_reinit & MATRIX_CHANGED != 0 {
                av_bprintf(
                    &mut reason,
                    CString::new("display matrix changed, \0").unwrap().as_ptr(),
                );
            }
            ...
            if need_reinit & HWACCEL_CHANGED != 0 {
                av_bprintf(
                    &mut reason,
                    CString::new("hwaccel changed, \0").unwrap().as_ptr(),
                );
            }
    ...
    *graph = avfilter_graph_alloc();
    ...
    for ifp in &mut *ifps {
        loop {
            ...
            let mut tmp_frame_box = option.unwrap();
...
unsafe fn configure_output_audio_filter(
    ...
    ret = insert_trim(ofp.opts.trim_start_us, ofp.opts.trim_duration_us,
                      &mut last_filter, &mut pad_idx, &name);
    if ret < 0 {
        return ret;
    }
...
fn insert_trim(start_time: Option<i64>, duration: Option<i64>,
               last_filter: *mut *mut AVFilterContext, pad_idx: &mut i32, filter_name:&str) -> i32 {
    ...
    unsafe {
        ...
        let ctx = ffmpeg_sys_next::avfilter_graph_alloc_filter(graph, trim, filter_name_cstring.as_ptr());
        if ctx.is_null() {
            return AVERROR(ENOMEM);
        }
...
unsafe fn fg_read_frames(
    ...
    loop {
        ...
        for ofp in &mut *ofps {
            let mut ret = 0;
            loop {
                ret = fg_output_step(fgp, ofp, frame_pool);
                if ret < 0 {
                    return ret;
                }
                if ret != 0 {
...
fn choose_input(ifps: &Vec<InputFilterParameter>) -> usize {
    ...
    for ifp in ifps {
        ...
        nb_requests = unsafe { av_buffersrc_get_nb_failed_requests(ifp.filter) } as i32;
...
unsafe fn video_sync_process(
    ...
    let mut sync_ipts = adjust_frame_pts_to_encoder_tb(frame, ofp.tb_out, ofp.opts.ts_offset.unwrap_or(0));
...
fn choose_sample_fmts(
    ...
    unsafe {
        av_bprintf(bprint, CString::new("sample_fmts=").unwrap().as_ptr());

        if format != AV_SAMPLE_FMT_NONE {
            let format_name = av_get_sample_fmt_name(format);
            if !format_name.is_null() {
                av_bprintf(bprint, CString::new("%s").unwrap().as_ptr(), format_name);
            }
...
fn choose_sample_rates(bprint: &mut AVBPrint, rate: i32, rates: Option<Vec<i32>>) {
    ...
    unsafe {
        av_bprintf(bprint, CString::new("sample_rates=").unwrap().as_ptr());

        if rate != 0 {
            av_bprintf(bprint, CString::new("%d").unwrap().as_ptr(), rate);
        } else if let Some(rates) = rates {
            for r in rates {
                av_bprintf(bprint, CString::new("%d|").unwrap().as_ptr(), r);
            }
            if bprint.len > 0 {
                let last_char_ptr = bprint.str_.add(bprint.len as usize - 1);
                *last_char_ptr = 0; // Remove last '|'
...
fn choose_channel_layouts(
    ...
    unsafe {
        ...
        if ret != 0 {
            av_bprintf(bprint, CString::new("channel_layouts=").unwrap().as_ptr());
            ...
            for l in layouts {
                ...
                if !bprint.str_.is_null() {
                    av_bprintf(bprint, CString::new("|").unwrap().as_ptr());
                }
            }
            if bprint.len > 0 {
                let last_char_ptr = bprint.str_.add(bprint.len as usize - 1);
...
fn choose_pix_fmts(
    ...
    unsafe {
        av_bprintf(bprint, CString::new("pix_fmts=").unwrap().as_ptr());
...
fn choose_color_spaces(
    ...
    unsafe {
        av_bprintf(bprint, CString::new("color_spaces=").unwrap().as_ptr());
...
fn choose_color_ranges(
    ...
    unsafe {
        av_bprintf(bprint, CString::new("color_ranges=").unwrap().as_ptr());
...
unsafe fn configure_input_audio_filter(
    ...
    av_bprintf(
        &mut args,
        CString::new("time_base=%d/%d:sample_rate=%d:sample_fmt=%s")
            .unwrap()
            .as_ptr(),
        (*ifp).time_base.num,
        (*ifp).time_base.den,
        ...
        && (*ifp).ch_layout.order != AV_CHANNEL_ORDER_UNSPEC
    {
        ...
        av_bprintf(
            &mut args,
            CString::new(":channels=%d").unwrap().as_ptr(),
...
fn insert_filter(
    ...
    let filter = unsafe { avfilter_get_by_name(CString::new(filter_name).unwrap().as_ptr()) };
    if filter.is_null() {
        return AVERROR_BUG;
    }
    ...
    let ret = unsafe {
        avfilter_graph_create_filter(
            &mut ctx,
            filter,
            CString::new(filter_name).unwrap().as_ptr(),
            args_ptr,
            std::ptr::null_mut(),
            graph,
        )
...
unsafe fn graph_parse(
    ...
    let desc = CString::new(graph_desc.clone()).expect("CString::new failed");
...
unsafe fn filter_opt_apply(f: *mut AVFilterContext, mut key: *mut c_char, val: *mut c_char) -> i32 {
    ...
    let mut o = null();
    ret == AVERROR_OPTION_NOT_FOUND && *key == b'/' as c_char;
    {
        o = av_opt_find(
